...
JAVA_WRAPPER Java_com_re3_game_TouchControlsView_nativeSetStick(JNIEnv *env, jobject obj, jint stick, jfloat x, jfloat y)
{
	if (stick == 0) {
...
JAVA_WRAPPER Java_com_re3_game_TouchControlsView_nativeSetMenuMouse(JNIEnv *env, jobject obj, jfloat x, jfloat y, jboolean down)
{
...
JAVA_WRAPPER Java_com_re3_game_TouchControlsView_nativeSetButton(JNIEnv *env, jobject obj, jint button, jboolean pressed)
{
	switch (button) {
...
// skip function ourselves.
extern "C" JNIEXPORT void JNICALL
Java_com_re3_game_TouchControlsView_nativeSkipCutscene(JNIEnv *env, jobject obj)
{
...
Java_com_re3_game_TouchControlsView_nativeGetGameContext(JNIEnv *env, jobject obj)
{
	if (FrontEndMenuManager.GetIsMenuActive())
		return 0;

	// Cutscenes take control away from the player entirely; showing movement/
	// action buttons during one is just confusing.
	if (CCutsceneMgr::IsRunning())
		return 3;
