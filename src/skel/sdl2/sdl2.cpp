...
#if defined ANDROID
    // Every relative fopen()/open() in the engine (gamefiles, .img streaming
    // in CdStream_posix.cpp, etc.) resolves against the process's cwd --
    // CFileMgr::Initialise() below just captures whatever that is via
    // getcwd() as its root. On Android there's no meaningful cwd (nothing
    // sets one, and it isn't the writable/visible game folder either), so
    // without this every one of those relative opens fails silently. chdir()
    // here, before Initialise() runs, makes the game folder --dir pointed at
    // (StorageRootBuffer, set from "--dir" at the top of main()) that root
    // for the whole engine, with zero changes needed anywhere else.
    if (StorageRootBuffer != nullptr)
        chdir(StorageRootBuffer);
#endif
...
void _InputInitialiseJoys()
{
    ...
    if (EnvControlConfig != nil) {
        ...
        if (SDL_GameControllerAddMappingsFromFile(SDL_GAMEPAD_DB_PATH) <= 0) {
            debug ("You don't seem to have copied %s file from re3/gamefiles "
                   "to GTA III directory. Some gamepads may not be recognized.\n",
                   SDL_GAMEPAD_DB_PATH);
        }
    }
...
main(int argc, char *argv[])
{
    ...
    for(i = 1; i < argc; i++)
    {
        if(strcmp(argv[i], "--dir") == 0 && i + 1 < argc) {
            ...
            // Every relative fopen()/open() in the engine (gamefiles, .img
            // streaming in CdStream_posix.cpp, etc.) resolves against the
            // process's cwd, and there's no meaningful one set on Android.
            // This has to happen here, before RsEventHandler(rsINITIALIZE, ...)
            // below -- that call reaches AppEventHandler() (core/main.cpp),
            // which runs CGame::InitialiseOnceBeforeRW() -> CdStreamInit()
            // and its relative statvfs("models/gta3.img", ...) immediately,
            // well before psInitialize() (which used to be where this chdir
            // lived) ever gets a turn.
            chdir(StorageRootBuffer);
#endif
        }
    }
...
Java_com_re3_game_TouchControlsView_nativeIsControllerConnected(JNIEnv *env, jobject obj)
{
    return gamepad1 != nullptr;
}

// re3 (this repo) never grew SDL2 support upstream -- ControllerConfig.cpp's
// mappedButtons[] scheme (MAX_BUTTONS = 17 slots) was built entirely around
// GLFW's GLFW_GAMEPAD_BUTTON_* ordering (see MapIdToButtonId() in
// ControllerConfig.cpp), which does NOT match SDL_CONTROLLER_BUTTON_*'s
// ordering past index 3 (SDL puts BACK at 4, GLFW puts LEFT_BUMPER there).
// These are the GLFW indices MapIdToButtonId() actually switches on, kept
// here as plain ints (not the GLFW_GAMEPAD_BUTTON_* macros) since GLFW itself
// isn't linked into the Android build at all.
enum {
    GAME_BTN_A = 0, GAME_BTN_B = 1, GAME_BTN_X = 2, GAME_BTN_Y = 3,
    GAME_BTN_LEFT_BUMPER = 4, GAME_BTN_RIGHT_BUMPER = 5,
    GAME_BTN_BACK = 6, GAME_BTN_START = 7, /* GAME_BTN_GUIDE = 8 unused */
    GAME_BTN_LEFT_THUMB = 9, GAME_BTN_RIGHT_THUMB = 10,
    GAME_BTN_DPAD_UP = 11, GAME_BTN_DPAD_RIGHT = 12, GAME_BTN_DPAD_DOWN = 13, GAME_BTN_DPAD_LEFT = 14,
    GAME_BTN_L2 = 15, GAME_BTN_R2 = 16, // MapIdToButtonId() reads these as manually-injected trigger "buttons"
};

// Feeds the on-screen touch controls (TouchControls.cpp/TouchControlsView.java)
// into the same CControllerState/PCTempJoyState a physical controller would,
// so every context that already handles gamepad input (driving, on foot,
// menus, ...) picks it up for free -- no separate action mapping needed.
void CaptureTouchPad(RwInt32 padID)
{
    ...
    ControlsManager.m_NewState.numButtons = JOY_BUTTONS;
    ...
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_B]            = g_TouchState.circle;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_A]            = g_TouchState.cross;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_X]            = g_TouchState.square;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_Y]            = g_TouchState.triangle;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_LEFT_BUMPER]  = g_TouchState.leftShoulder1;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_RIGHT_BUMPER] = g_TouchState.rightShoulder1;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_L2]           = g_TouchState.leftShoulder2;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_R2]           = g_TouchState.rightShoulder2;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_BACK]         = g_TouchState.select;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_START]        = g_TouchState.start;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_LEFT_THUMB]   = g_TouchState.leftStickClick;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_RIGHT_THUMB]  = g_TouchState.rightStickClick;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_DPAD_UP]      = g_TouchState.dpadUp;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_DPAD_DOWN]    = g_TouchState.dpadDown;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_DPAD_LEFT]    = g_TouchState.dpadLeft;
    ControlsManager.m_NewState.mappedButtons[GAME_BTN_DPAD_RIGHT]   = g_TouchState.dpadRight;

    CPad *pad = CPad::GetPad(padID);

    // 0.3f deadzone / *128.0f scaling matches CapturePad()'s own convention
    // for a real controller below (and skel/glfw/glfw.cpp's PC equivalent) --
    // this build has no per-axis sensitivity/deadzone settings to read.
    if (Abs(g_TouchState.leftX) > 0.3f)
        pad->PCTempJoyState.LeftStickX = (int32)(g_TouchState.leftX * 128.0f);

    if (Abs(g_TouchState.leftY) > 0.3f)
        pad->PCTempJoyState.LeftStickY = (int32)(g_TouchState.leftY * 128.0f);

    if (Abs(g_TouchState.rightX) > 0.3f)
        pad->PCTempJoyState.RightStickX = (int32)(g_TouchState.rightX * 128.0f);

    if (Abs(g_TouchState.rightY) > 0.3f)
        pad->PCTempJoyState.RightStickY = (int32)(g_TouchState.rightY * 128.0f);
...
void CapturePad(RwInt32 padID)
{
    ...
    if (ControlsManager.m_bFirstCapture == false) {
        ...
        ControlsManager.m_NewState.mappedButtons[GAME_BTN_L2] = 0;
        ControlsManager.m_NewState.mappedButtons[GAME_BTN_R2] = 0;
    }

    // SDL_CONTROLLER_BUTTON_* doesn't share GLFW's ordering past index 3 (see
    // the GAME_BTN_* comment above CaptureTouchPad) -- translate each one to
    // the slot MapIdToButtonId() actually expects it in. -1 = no game action
    // uses that physical button, skip it. Bounded to MAX_BUTTONS (17) slots;
    // SDL_CONTROLLER_BUTTON_MAX (21, once misc/paddle/touchpad buttons are
    // counted) would overflow mappedButtons[] if used as the loop bound.
    static const int8 kSdlToGameButton[SDL_CONTROLLER_BUTTON_DPAD_RIGHT + 1] = {
        GAME_BTN_A, GAME_BTN_B, GAME_BTN_X, GAME_BTN_Y,             // SDL A,B,X,Y
        GAME_BTN_BACK, -1, GAME_BTN_START,                         // SDL BACK,GUIDE,START
        GAME_BTN_LEFT_THUMB, GAME_BTN_RIGHT_THUMB,                 // SDL LEFTSTICK,RIGHTSTICK
        GAME_BTN_LEFT_BUMPER, GAME_BTN_RIGHT_BUMPER,                // SDL LEFTSHOULDER,RIGHTSHOULDER
        GAME_BTN_DPAD_UP, GAME_BTN_DPAD_DOWN, GAME_BTN_DPAD_LEFT, GAME_BTN_DPAD_RIGHT,
    };

    // buttons is uint8* in this build's GlfwJoyState (backed by whatever the
    // platform's joystick query returns, see skel/glfw/glfw.cpp), not a fixed
    // array -- point it at storage that outlives this function.
    static uint8 sdlButtonStates[SDL_CONTROLLER_BUTTON_DPAD_RIGHT + 1];
    for (int i = 0; i <= SDL_CONTROLLER_BUTTON_DPAD_RIGHT; ++i) {
        int state = SDL_GameControllerGetButton(gamepad, (SDL_GameControllerButton) i);
        assert(state == 0 || state == 1);
        sdlButtonStates[i] = (uint8)state;

        int gameBtn = kSdlToGameButton[i];
        if (gameBtn >= 0)
            ControlsManager.m_NewState.mappedButtons[gameBtn] = !!state;
    }
    ControlsManager.m_NewState.buttons = sdlButtonStates;

    ControlsManager.m_NewState.numButtons = SDL_CONTROLLER_BUTTON_DPAD_RIGHT + 1;
    ...
    if (ControlsManager.m_NewState.isGamepad) {
        ...
        if (lt != 0.0f)
            ControlsManager.m_NewState.mappedButtons[GAME_BTN_L2] = lt > -0.8f;

        if (rt != 0.0f)
            ControlsManager.m_NewState.mappedButtons[GAME_BTN_R2] = rt > -0.8f;
    }
